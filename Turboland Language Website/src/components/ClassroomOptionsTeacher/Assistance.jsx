...
function Assistance() {
  ...
  const {email} = studentStore()
  function obtenerAsistencias(asistencias, email) {
    return asistencias.map(asistencia => {
      const alumno = asistencia.alumnos.find(alumno => alumno.email === email);
      if (alumno) {
        return { ...asistencia, alumnos: alumno };
      } else {
        return { ...asistencia, alumnos: { email: email, estado: 'none' } };
      }
    });
  }
  ...
  return (
    <div className="flex flex-col justify-between items-center bg-random1 bg-contain h-[100vh] w-full">
      ...
      <div className="flex justify-evenly w-full  h-[80%]">
        ...
        <button onClick={()=>navigate("/teacherDashboard/AssistanceActivities",{state:{asistencias,cache:location?.state?.notasYAsistencias}})} className="bg-[#849652]  border-[8px] border-[#171717] rounded-lg p-5  hover:scale-105 flex flex-col items-center justify-center h-[500px]">
          <img draggable={false}
          width={350}
          height={350}
          ...
          <div className="w-full text-center bg-[#171717] py-5 rounded-tl-lg rounded-br-lg mt-3">
            ...
            Ver la asistencia de los estudiantes
            </span>
          </div>
        </button>

        <button onClick={()=>navigate("/teacherDashboard/Assistance/MyNotes",{state:{notas,cache:location?.state?.notasYAsistencias}})} className="bg-[#849652] border-[8px] border-[#171717] rounded-lg p-5  hover:scale-105 h-[500px]">
          <img draggable={false}
          width={350}
          height={350}
          ...
          <div className="w-full text-center bg-[#171717] py-5 rounded-tl-lg rounded-br-lg mt-3">
            <span className="text-[#fff] text-xl font-semibold ">
              Pon notas a tus estudiantes aquí!!
            </span>
          </div>
        </button>
