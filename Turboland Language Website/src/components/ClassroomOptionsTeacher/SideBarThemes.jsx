...
function SideBarThemes({selected,setSelected,data}) {
  
  const navigate = useNavigate();
  let sorted = data.sort((a, b) => a.lapso.localeCompare(b.lapso))
  return (
  ...
  <nav class="hs-accordion-group p-4 w-full flex flex-col flex-wrap" data-hs-accordion-always-open >
    ...
    {sorted?.map((item,index)=>{
      return(
      <ThemesItem key={`Themes`+index} data={item} setSelected={setSelected} selected={selected} />
    ) })}
    </ul>
  </nav>
