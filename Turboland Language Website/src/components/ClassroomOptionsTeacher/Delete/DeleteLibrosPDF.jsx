...
function DeleteLibrosPDF({id,data}) {
  const navigate = useNavigate();
  const handleSubmit = async()=>{
    if(data){
        const {status} = await deleteClassroom("librosPDF",data,id)
        if(status === 200){
          toast.success("Libro eliminado correctamente")
          navigate("/teacherDashboard")
        }else{
          toast.error("Ha ocurrido un error al eliminar el libro")
        }
...
export default DeleteLibrosPDF