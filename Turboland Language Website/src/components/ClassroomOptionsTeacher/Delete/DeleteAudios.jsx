...
function DeleteAudios({id,data}) {
  const navigate = useNavigate();
  const handleSubmit = async()=>{
    if(data){
        const {status} = await deleteClassroom("audios",data,id)
        if(status === 200){
          toast.success("Audio eliminado correctamente")
          navigate("/teacherDashboard")
        }else{
          toast.error("Ha ocurrido un error al eliminar el audio")
        }
