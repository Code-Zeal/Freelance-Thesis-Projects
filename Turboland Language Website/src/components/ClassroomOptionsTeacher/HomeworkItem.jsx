...
function HomeworkItem({selected,setSelected,data}) {
  return (
    <button onClick={selected?.url === data?.url? false:()=>setSelected(data)} class={selected?.url === data?.url?"hover:cursor-not-allowed flex group bg-gray-400 border shadow-sm rounded-xl overflow-hidden transition w-[98%] mx-[1%] h-auto my-2 items-center justify-center px-2":"hover:cursor-pointer flex group bg-white border shadow-sm rounded-xl overflow-hidden hover:shadow-lg transition w-[98%] mx-[1%] h-auto my-2 items-center justify-center px-2"} >
    <div class=" w-[80%]">
