...
function SideBarAudios({selected,setSelected,data}) {
  ...
  <nav class="hs-accordion-group p-4 w-full flex flex-col flex-wrap" data-hs-accordion-always-open >
    ...
    {sorted?.map((item,index)=>{
      return(
      <AudioItem key={`AudioNro`+index} data={item} setSelected={setSelected} selected={selected} />
    ) })}
    </ul>
  </nav>
