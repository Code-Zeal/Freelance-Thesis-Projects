...
function NewVideoModal({setModal,data,id,notasYAsistencias}) {
  ...
  const handleSubmit = async()=>{
    ...
    if(titleTest && urlTest){
      ...
      let finalData = {
        ...
        const {status} = await updateClassroom("videos" ,finalData,id,false,notasYAsistencias)
        if(status === 200){
          ...
          setLoading(false)
          toast.error("Ha ocurrido un error al agregar el video ")
        }
    }else{
      setLoading(false)
  ...
  return (
    <div className='w-[70vw] h-[95vh] left-0 right-0 top-0 bottom-0 ml-auto mr-auto mt-auto mb-auto bg-gray-100 absolute z-50 text-black rounded-lg p-5 flex flex-col'>
          <div className="flex w-full justify-between mt-2 items-center">
          <h1 class="text-4xl dark:text-white">Crear nuevo video</h1>
    ...
    autofill:pb-2" placeholder="you@email.com"/>
      ...
      peer-[:not(:placeholder-shown)]:text-gray-500 dark:peer-[:not(:placeholder-shown)]:text-neutral-500 dark:text-neutral-500">Nombre del video</label>
      </div>
      <div class="relative">
    ...
    autofill:pb-2" placeholder="you@email.com"/>
      ...
      peer-[:not(:placeholder-shown)]:translate-x-0.5
      peer-[:not(:placeholder-shown)]:-translate-y-1.5
      peer-[:not(:placeholder-shown)]:text-gray-500 dark:peer-[:not(:placeholder-shown)]:text-neutral-500 dark:text-neutral-500">Link del video de youtube</label>
      </div>
