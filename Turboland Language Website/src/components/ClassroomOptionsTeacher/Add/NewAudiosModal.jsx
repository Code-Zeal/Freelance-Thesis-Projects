...
function NewAudiosModal({setModal,data,id,notasYAsistencias}) {
  ...
  const handleSubmit = async()=>{
    ...
    if(titleTest && file){
      let finalData = {
        ...
        const {status} = await updateClassroom("audios" ,finalData,id,file,notasYAsistencias)
        if(status === 200){
          toast.success("Audio creado correctamente")
          setModal(false)
          setLoading(false)
          navigate("/teacherDashboard")
        }else{
          setLoading(false)
          toast.error("Ha ocurrido un error al crear el audio")
        }
    }else{
      setLoading(false)
  ...
  return (
    <div className='w-[75vw] h-[95vh] left-0 right-0 top-0 bottom-0 ml-auto mr-auto mt-auto mb-auto bg-gray-100 absolute z-50 text-black rounded-lg p-5 flex flex-col'>
          ...
          <h1 class="text-4xl dark:text-white">Crear nuevo Audio</h1>
    ...
    autofill:pb-2" placeholder="you@email.com"/>
      ...
      peer-[:not(:placeholder-shown)]:text-gray-500 dark:peer-[:not(:placeholder-shown)]:text-neutral-500 dark:text-neutral-500">Nombre del audio</label>
      </div>
      <div class="w-[60%] ml-auto mr-auto">
    <label class="large-file-input block mt-20 ">
      <span class="sr-only">Sube el audio</span>
      <input name="audio-file-input" id="audio-file-input" type="file" accept="audio/*"
