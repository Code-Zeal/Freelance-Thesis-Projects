...
function NewTestModal({setModal,data,id,notasYAsistencias}) {
  ...
  const handleSubmit = async()=>{
    ...
    if(titleTest && urlTest){
      let finalData = {
        ...
        const {status} = await updateClassroom("evaluaciones",finalData,id,false,notasYAsistencias)
        if(status === 200){
          ...
          setLoading(false)
          toast.error("Ha ocurrido un error al crear el examen")
        }
    }else{
      setLoading(false)
  ...
  return (
    <div className='w-[75vw] h-[95vh] left-0 right-0 top-0 bottom-0 ml-auto mr-auto mt-auto mb-auto bg-gray-100 absolute z-50 text-black rounded-lg p-5 flex flex-col'>
          ...
          <h1 class="text-4xl dark:text-white">Crear nuevo examen</h1>
    ...
    autofill:pb-2" placeholder="you@email.com"/>
      ...
      peer-[:not(:placeholder-shown)]:text-gray-500 dark:peer-[:not(:placeholder-shown)]:text-neutral-500 dark:text-neutral-500">Nombre del examen</label>
      </div>
      <div class="relative">
