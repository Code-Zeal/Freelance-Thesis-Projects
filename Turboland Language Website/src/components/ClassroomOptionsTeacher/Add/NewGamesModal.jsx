...
function NewGamesModal({setModal,data,id,notasYAsistencias}) {
  ...
  const handleSubmit = async()=>{
    ...
    if(titleTest && urlTest){
      let finalData={   
        ...
        const {status} = await updateClassroom("juegos" ,finalData,id,false,notasYAsistencias)
        if(status === 200){
          ...
          setLoading(false)
          toast.error("Ha ocurrido un error al crear el juego")
        }
    }else{
      setLoading(false)
  ...
  return (
    <div className='w-[75vw] h-[95vh] left-0 right-0 top-0 bottom-0 ml-auto mr-auto mt-auto mb-auto bg-gray-100 absolute z-50 text-black rounded-lg p-5 flex flex-col'>
          ...
          <h1 class="text-4xl dark:text-white">Crear nuevo juego</h1>
    ...
    autofill:pb-2" placeholder="you@email.com"/>
      ...
      peer-[:not(:placeholder-shown)]:text-gray-500 dark:peer-[:not(:placeholder-shown)]:text-neutral-500 dark:text-neutral-500">Nombre del juego</label>
      </div>


      <div class="relative">
          <input value={descriptionGame} onChange={(e)=>setDescriptionGame(e.target.value)} type="email" id="hs-floating-input-email" class="peer p-4 block w-full mr-auto mt-12 border-gray-200 rounded-lg text-sm placeholder:text-transparent focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:focus:ring-neutral-600
    focus:pt-6
    focus:pb-2
    [&:not(:placeholder-shown)]:pt-6
    [&:not(:placeholder-shown)]:pb-2
    ...
    autofill:pb-2" placeholder="you@email.com"/>
      ...
      peer-[:not(:placeholder-shown)]:translate-x-0.5
      peer-[:not(:placeholder-shown)]:-translate-y-1.5
      peer-[:not(:placeholder-shown)]:text-gray-500 dark:peer-[:not(:placeholder-shown)]:text-neutral-500 dark:text-neutral-500">Descripción del juego</label>
      </div>
      <div class="relative">
    ...
    autofill:pb-2" placeholder="you@email.com"/>
      ...
      peer-[:not(:placeholder-shown)]:translate-x-0.5
      peer-[:not(:placeholder-shown)]:-translate-y-1.5
      peer-[:not(:placeholder-shown)]:text-gray-500 dark:peer-[:not(:placeholder-shown)]:text-neutral-500 dark:text-neutral-500">Link del juego</label>
      </div>
