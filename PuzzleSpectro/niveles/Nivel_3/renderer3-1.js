...
const detenerContador = () => {
  ...
  localStorage.setItem("level3-1", `${min} minutos y ${seg} segundos`);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  nextBtn.addEventListener("click", function () {
    ipcRenderer.send("navigate", "niveles/Nivel_3/nivel3-2.html");
  });
...
Puzzle.prototype.mouseUpGame = function (event) {
  ...
  var bg = document.getElementById("level3"); // asegúrate de que este es el ID de tu elemento de audio
  bg.pause();
...
window.addEventListener("load", function () {
  ...
  const imagePath = path.join(__dirname, "level3-1.jpeg"); // Cambia la ruta según sea necesario.
