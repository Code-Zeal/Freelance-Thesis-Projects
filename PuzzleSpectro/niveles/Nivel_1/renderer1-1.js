  ...
  intervalo = setInterval(() => {
    ...
    if (contador) {
      document.getElementById(
        "contador"
      ).textContent = `${min} minutos y ${seg} segundos`;
    }
...
const detenerContador = () => {
  ...
  localStorage.setItem("level1-1", `${min} minutos y ${seg} segundos`);
  const timer = document.getElementById("timerText");
  timer.textContent = "En: " + min + " minutos y " + seg + " segundos";
};

// Ejecutar la función cuando la ventana esté lista

document.addEventListener("click", function () {
  var audio = document.getElementById("grabSound"); // asegúrate de que este es el ID de tu elemento de audio
  audio.play();
});
// Obtén una referencia al botón y al contenedor de la imagen
document.addEventListener("DOMContentLoaded", function () {
  setTimeout(ejecutarContador, 4000);
  const helpBtn = document.getElementById("helpBtn");
  const adviceContainer = document.getElementById("adviceContainer");
  const closeBtn = document.getElementById("closeBtn");
  const nextBtn = document.getElementById("nextBtn");
  nextBtn.addEventListener("click", function () {
    ipcRenderer.send("navigate", "niveles/Nivel_1/nivel1-2.html");
  });
...
Side.prototype.drawPath = function (
  ctx,
  shiftx,
  shifty,
  backwards,
...
Piece.prototype.createDivPiece = function (puzzle, scale, offsx, offsy) {
  ...
  this.moveTo(
    new Point(
      (this.kx - 1) * puzzle.dx + Puzzle.MARGIN1,
...
Puzzle.prototype.merge = function (n1, n2) {
  ...
  for (
    k = 0;
    k < this.polyPieces.length && this.polyPieces[k].pieces.length >= nbpieces;
    k++
  ) {}
  // insert new
...
Puzzle.prototype.mouseUpGame = function (event) {
  ...
  console.log("ganaste");
  detenerContador();
  const winModal = document.getElementById("winContainer");

  winModal.style.display = "flex";
  var bg = document.getElementById("level1"); // asegúrate de que este es el ID de tu elemento de audio
  bg.pause();
  var win = document.getElementById("winSound"); // asegúrate de que este es el ID de tu elemento de audio
  win.play();
...
// Esta función calcula la cantidad de piezas en el eje x y y basado en un número deseado de piezas
Puzzle.prototype.computenxAndny = function () {
  let aspectRatio = this.image.width / this.image.height;
  let area = this.reqWidth * this.reqHeight;
  let pieceArea = area / this.npieces; // Este valor determina el tamaño de las piezas
  let pieceAspectRatio = Math.sqrt(aspectRatio); // Proporción para mantener el aspecto del rompecabezas

  // Calculamos nx y ny como la raíz cuadrada del número de piezas adaptada a la proporción del aspecto
  this.nx = Math.round(Math.sqrt(this.npieces) * pieceAspectRatio);
  this.ny = Math.round(this.npieces / this.nx);

  // Aseguramos que el producto de nx y ny sea mayor o igual que el número deseado de piezas
  while (this.nx * this.ny < this.npieces) {
    if (this.nx <= this.ny) {
      this.nx += 1;
    } else {
      this.ny += 1;
...
window.addEventListener("load", function () {
  ...
  // __dirname proporciona la ruta del directorio actual en Node.js si tienes la imagen en el mismo directorio que tu script.
  // Si tu imagen está en una carpeta llamada "images" en el mismo directorio que tu script de Electron, por ejemplo, puedes hacer:
  const imagePath = path.join(__dirname, "level1-1.jpeg"); // Cambia la ruta según sea necesario.

  autoStart = true;
  const puzzleGame = new Puzzle({
    img: imagePath, // Usamos la ruta local en lugar de una URL.
    width: window.innerWidth,
    height: window.innerHeight,
    idiv: "forPuzzle",
  });

  puzzleGame.npieces = 6;

  // Crear el rompecabezas
  puzzleGame.createPuzzle({
    img: puzzleGame.image,
    height: puzzleGame.reqHeight,
    width: puzzleGame.reqWidth,
    idiv: puzzleGame.divGame,
  });
});
