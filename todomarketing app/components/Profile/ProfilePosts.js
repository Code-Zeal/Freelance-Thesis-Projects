  ...
  useFocusEffect(
    React.useCallback(() => {
      if (profileSection === "publicaciones") {
        if (entrepreneurship && entrepreneurship[0]?.posts?.length > 0) {
          getPostData();
        }
      } else if (entrepreneurship && entrepreneurship[0]?.posts?.length > 0) {
        getVideosData();
      }
    ...
    <View style={styles.container}>
      ...
      >
        {postsData &&
          ...
          <Text>No tienes publicaciones o videos...</Text>
        )}
      </ScrollView>
    </View>
  );
